...
- 단평/단원평가/단평4/단원평가1/단원평가-단항식 등 → **기본은 textbook="단원평가"로 통합**
  ...
  - **번호 회차(단원평가3, 단원평가4) 처리 규칙 — 매우 중요!**:
    - 같은 그룹 내에 단원평가3과 단원평가4가 동시에 등장하면 **회차별로 구분 유지**:
      "단원평가3", "단원평가4"로 각각 분리
    - **대세(dominant) 회차 편입 규칙**: 한 학생이 회차 없이 "단원평가 40" 같이 벙벙하게 썼는데
      같은 그룹에 명시된 회차가 여러 종류 있으면, **가장 많은 학생이 쓴 회차(대세)로 편입**.
      예: 그룹에 단원평가4(8명) + 단원평가3(2명, 놓친 뒷부분 꼼꼼하게 복습 중)이 있고
      한 학생이 "단원평가 40"이라고만 쓰면 → 단원평가4로 편입. corrections에
      "[학생] 단원평가 40 → 단원평가4 40으로 추정(그룹 대세)" 기록.
    - 반대로 소수(단원평가3 2명)는 자기가 명시한 회차 그대로 유지(꼼꼼한 복습으로 간주).
    - 대세가 모호(1:1 등)하면 "단원평가"로 두고 corrections에 "회차 불명, 확인 필요" 기록.
