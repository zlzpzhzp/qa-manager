# 작업 맥락 (QA Manager)
> 마지막 갱신: 2026-04-18 12:05

## 2026-04-18 12:03~ PIN 인증 제거 작업 진행 중
- 선생님 지시 (12:02 KST): "핀번호 없애" → 확인 → "야 그냥 없애 주소 조심히 쓰면됨"
- 현재 PIN: 0999 (Vercel env TEACHER_PIN, 끝에 \n 붙어있음)
- 삭제 대상:
  - src/middleware.ts (삭제됨)
  - src/app/login/ (삭제됨)
  - src/app/api/auth/ (삭제됨, logout 포함)
  - __tests__/auth.test.ts (삭제됨)
  - submissions/route.ts checkAuth 블록 (삭제됨)
- 남은 작업:
  - supabase.ts의 verifyAuthToken/signToken/createAuthToken 제거
  - 빌드 확인
  - 테스트 통과 확인
  - 커밋 + vercel --prod 배포
  - 텔레그램 보고

## 2026-04-18 03:02 프롬프트 개선 2차 (1,2번) 완료
- 선생님 지시: "그래 1,2번부터 해"
- 1. 단원평가 하위 표기 통합 (단원평가1, -단항식, -순환소수, 단평1 → 모두 단원평가)
     - 단, 단원평가1/2 동시 등장 시 구분 유지
- 2. 학년-학기 맥락 상속 확장 ("2-1", "3-1"도 숫자만 규칙처럼 교재 상속)
- 3. 중복 corrections 방지 규칙 추가
- 검증: 04-17 중2S 단원평가 5종 → 단원평가/단원평가2 2종 통합 성공
- 커밋 d99c0ab, 배포 dm-qa.vercel.app (36s)

## 2026-04-18 02:50 프롬프트 개선 1차 (4개) 완료 + 배포
- 커밋 7015332
- 4개 블록 추가: 숫자 상속, 노이즈 명시, 학년-학기 일관, 범위 '~' 대조→fallback
- 검증: 185건 재분석, "쎈 11111111" 등 corrections 기록, "678"→쎈 상속, p24~25→24p 통일
- 배포 성공 (34s)
